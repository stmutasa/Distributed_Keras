...
class SODLoader():
    ...
    def load_tfrecords(self, dataset, data_dims=[], image_dtype=tf.float32, segments='label_data',
        ...
        feature_dict = {'id': tf.io.FixedLenFeature([], tf.int64)}
        for key, value in loaded_dict.items(): feature_dict[key] = tf.io.FixedLenFeature([], tf.string)
        ...
        features = tf.io.parse_single_example(serialized=dataset, features=feature_dict)
        ...
        for key, value in loaded_dict.items():
            ...
            if 'bbox' in key:
                data[key] = tf.io.decode_raw(features[key], tf.float32)
                data[key] = tf.reshape(data[key], shape=[-1, 5])
                # data[key] = tf.cast(data[key], tf.float32)

            elif segments in key:
                data[key] = tf.io.decode_raw(features[key], segments_dtype)
                data[key] = tf.reshape(data[key], shape=segments_shape)
                # data[key] = tf.cast(data[key], tf.float32)

            elif 'data' in key:
                data[key] = tf.io.decode_raw(features[key], image_dtype)
                ...
                data[key] = tf.strings.to_number(features[key], tf.float32)
    ...
    def load_tfrecords_dataset(self, filenames):
        ...
        filename_queue = tf.compat.v1.train.string_input_producer(filenames, num_epochs=None)

        reader = tf.compat.v1.TFRecordReader()  # Instantializes a TFRecordReader which outputs records from a TFRecords file
        ...
        # Parses one protocol buffer file into the features dictionary which maps keys to tensors with the data: 'key': parse_single_eg
        features = tf.io.parse_single_example(serialized=serialized_example, features=feature_dict)
    ...
    def randomize_batches(self, image_dict, batch_size, dynamic=False):
        ...
        if dynamic:
            shuffled = tf.compat.v1.train.batch(tensors, batch_size=batch_size, capacity=capacity, dynamic_pad=True)
        else:
            shuffled = tf.compat.v1.train.shuffle_batch(tensors, batch_size=batch_size, capacity=capacity, min_after_dequeue=min_dq)
    ...
    def val_batches(self, image_dict, batch_size):
        ...
        # This function creates batches by randomly shuffling the input tensors. returns a dict of shuffled tensors
        shuffled = tf.compat.v1.train.batch(tensors, batch_size=batch_size, capacity=capacity)
    ...
    def resize_3D_by_axis(self, image, dim_1, dim_2, axis_to_hold, is_grayscale):
        ...
        if is_grayscale:
            ...
            for i in unstack_img_depth_list:
                resized_list.append(tf.image.resize(i, [dim_1, dim_2], method=0))
    ...
    def save_tfrecords(self, data, xvals, test_size=50, file_root='data/Data'):
        ...
        if xvals == 1:
            ...
            writer = tf.io.TFRecordWriter((file_root + '.tfrecords'))
        ...
        for z in range(xvals):
            ...
            writer.append(tf.io.TFRecordWriter(filename))
