...
class DataPreprocessor(object):
    ...
    def __call__(self, record):
        ...
        if self._distords:  # Training
            ...
            record['data'] = tf.image.resize(record['data'], [256, 256])

            # Random gaussian noise
            T_noise = tf.random.uniform([], 0, 0.1)
            noise = tf.random.uniform(shape=[256, 256, 1], minval=-T_noise, maxval=T_noise)
...
def dummy_inputs(local_batch_size):
    ...
    dummy_labels = np.random.randint(0, 2, [200]).astype(np.float32)
    ...
    dataset= tf.data.Dataset.from_tensor((dummy_inputs, dummy_labels)).shuffle(200).repeat().batch(local_batch_size)
