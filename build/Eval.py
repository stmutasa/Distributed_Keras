...
def test():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        # Display the images
        tf.compat.v1.summary.image('Test IMG', tf.reshape(images[0], shape=[1, 256, 256, 1]), 4)
        ...
        var_init = tf.group(tf.compat.v1.global_variables_initializer(), tf.compat.v1.local_variables_initializer())
        ...
        saver = tf.compat.v1.train.Saver(var_restore, max_to_keep=3)
        ...
        while True:
            ...
            with tf.compat.v1.Session(config=config) as mon_sess:
