...
def main(_):
    ...
    if FLAGS.job_name == "ps":
        ...
        _ProfilerHook = tf.estimator.ProfilerHook(save_steps=max_steps // 8, output_dir='data/checkpoints/',
                                              show_memory=True, show_dataflow=True)

        # Make a summary hook
        summary_hook = tf.estimator.SummarySaverHook(save_steps=max_steps // 10, output_dir='data/checkpoints/', summary_op=all_summaries)

        # Group our hooks for the monitored train sessions.
        hooks = [tf.estimator.StopAtStepHook(last_step=max_steps), sync_replicas_hook, tf.estimator.NanTensorHook(train_op),
                 _ProfilerHook, _LoggerHook(), summary_hook]

        # Scafford object for finalizing the graph
        scaffold = tf.compat.v1.train.Scaffold(local_init_op=tf.group(tf.compat.v1.local_variables_initializer(), dataset_iterator.initializer))
...
def train_step(inputs, global_batch_size):
    ...
    # Display the images
    tf.compat.v1.summary.image('Train IMG', tf.reshape(images[0], shape=[1, 256, 256, 1]), 4)
...
def calculate_optimizer(loss):
    ...
    with tf.compat.v1.name_scope('train'):
        ...
        for var in tf.compat.v1.trainable_variables(): tf.compat.v1.summary.histogram(var.op.name, var)
