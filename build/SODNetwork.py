...
class SODMatrix(object):
    ...
    def convolution(self, scope, X, F, K, S=2, padding='SAME', phase_train=None, BN=True, relu=True, downsample=False, bias=True, dropout=None):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            kernel = tf.compat.v1.get_variable('Weights', shape=[F, F, C, K], initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))

            # Add to the weights collection
            tf.compat.v1.add_to_collection('weights', kernel)

            # Perform the actual convolution
            conv = tf.nn.conv2d(input=X, filters=kernel, strides=[1, S, S, 1], padding=padding)

            # Add in the bias
            if bias:
                bias = tf.compat.v1.get_variable('Bias', shape=[K], initializer=tf.compat.v1.constant_initializer(0.0))
                tf.compat.v1.add_to_collection('biases', bias)
            ...
            if dropout and phase_train == True: conv = tf.nn.dropout(conv, 1 - (dropout), noise_shape=[B, 1, 1, C])
    ...
    def convolution_3d(self, scope, X, F, K, S=2, padding='SAME', phase_train=None, BN=True, relu=True, downsample=False, dropout=None):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            try:
                kernel = tf.compat.v1.get_variable('Weights', shape=[F[0], F[1], F[2], C, K],
                                         initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
            except:
                kernel = tf.compat.v1.get_variable('Weights', shape=[F, F, F, C, K],
                                         initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
            ...
            # Channel wise Dropout
            if dropout and phase_train == True: conv = tf.nn.dropout(conv, 1 - (dropout), noise_shape=[B, 1, 1, 1, C])
    ...
    def depthwise_convolution(self, scope, X, F, K, S=2, padding='SAME', phase_train=None, BN=True, relu=True):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Define the Kernel. Can use Xavier init: contrib.layers.xavier_initializer())
            kernel = tf.compat.v1.get_variable('Weights', shape=[F, F, C, K],
                                     initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
            ...
            # Perform the actual convolution
            conv = tf.nn.depthwise_conv2d(input=X, filter=kernel, strides=[1, S, S, 1], padding=padding)
    ...
    def deconvolution(self, scope, X, F, K, S, padding='SAME', phase_train=None, concat=True,
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Xavier init
            kernel = tf.compat.v1.get_variable('Weights', shape=[F, F, K, C],
                                     initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
    ...
    def deconvolution_3d(self, scope, X, F, K, S, padding='SAME', phase_train=None, concat=True,
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Xavier init
            kernel = tf.compat.v1.get_variable('Weights', shape=[F, F, F, K, C],
                                     initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
    ...
    def inception_layer(self, scope, X, K, S=1, padding='SAME', phase_train=None, BN=True, relu=True, dropout=None):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            inception4 = tf.nn.max_pool2d(input=X, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding=padding)
            ...
            if dropout and phase_train == True: inception = tf.nn.dropout(inception, 1 - (dropout), noise_shape=[B, 1, 1, C])
    ...
    def inception_layer_3d(self, scope, X, K, Fz=1, S=1, padding='SAME', phase_train=None, BN=True, relu=True, dropout=None):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Apply channel wise dropout here
            if dropout and phase_train == True: inception = tf.nn.dropout(inception, 1 - (dropout), noise_shape=[B, 1, 1, 1, C])
    ...
    def residual_layer_stanford(self, scope, X, F, K, S=2, K_prob=None, padding='SAME', phase_train=None, DSC=False, BN=False, relu=False, dropout=None):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Apply channel wise dropout here
            if dropout and phase_train == True: residual = tf.nn.dropout(residual, 1 - (dropout), noise_shape=[B, 1, 1, C])
    ...
    def residual_layer_stanford_3d(self, scope, X, F, K, S=2, K_prob=None, padding='SAME',
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Apply channel wise dropout here
            if dropout and phase_train == True: residual = tf.nn.dropout(residual, 1 - (dropout), noise_shape=[B, 1, 1, 1, C])
    ...
    def fc7_layer(self, scope, X, neurons, dropout=False, phase_train=True, keep_prob=0.5, BN=False, relu=True, override=None, pad='VALID'):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            weights = tf.compat.v1.get_variable('weights', shape=[height * width * channel, neurons])

            # Define the biases
            bias = tf.compat.v1.get_variable('Bias', shape=[neurons], initializer=tf.compat.v1.constant_initializer(0.0))
            ...
            # Convolution
            conv = tf.nn.conv2d(input=X, filters=reshape, strides=[1, 1, 1, 1], padding=pad, name='Conv')
            ...
            if (phase_train == True) and dropout: conv = tf.nn.dropout(conv, 1 - (keep_prob))

            # Average outputs if we used an override
            if override:
                F = conv.get_shape().as_list()[1]
                conv = tf.nn.avg_pool2d(input=conv, ksize=[1, F, F, 1], strides=[1, 2, 2, 1], padding='VALID')
    ...
    def fc7_layer_3d(self, scope, X, neurons, dropout=False, phase_train=True, keep_prob=0.5,
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Initialize the weights
            weights = tf.compat.v1.get_variable('weights', shape=[height * width * depth * channel, neurons])
            ...
            # Convolution
            conv = tf.nn.conv3d(input=X, filters=reshape, strides=[1, 1, 1, 1, 1], padding=pad, name='Conv')
    ...
    def fc7_layer_old(self, scope, X, neurons, dropout=False, phase_train=True, keep_prob=0.5, BN=False):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Initialize the weights
            weights = tf.compat.v1.get_variable('weights', shape=[dim, neurons],
                                      initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
            ...
            # Dropout here if wanted and in train phase
            if (phase_train == True) and dropout: fc7 = tf.nn.dropout(fc7, 1 - (keep_prob))
    ...
    def linear_layer(self, scope, X, neurons, dropout=False, phase_train=True, keep_prob=0.5, BN=False, relu=True, add_bias=True):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            # Dropout here if wanted and in train phase
            if (phase_train == True) and dropout: linear = tf.nn.dropout(linear, 1 - (keep_prob))
    ...
    def spatial_transform_layer(self, scope, X):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            W1 = tf.compat.v1.get_variable('Weights1', shape=[dim * dim * 128, 20],
                                 initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
            B1 = tf.compat.v1.get_variable('Bias1', shape=[20], initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
            W2 = tf.compat.v1.get_variable('Weights2', shape=[20, 6], initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))

            # Add weights to collection
            tf.compat.v1.add_to_collection('weights', W1)
            tf.compat.v1.add_to_collection('weights', W2)
    ...
    def transitional_layer(self, scope, X, K, S=1, padding='SAME', phase_train=None, BN=True, relu=True):
        ...
        with tf.compat.v1.variable_scope(scope) as scope:
            ...
            inception1 = tf.nn.avg_pool2d(input=inception1a, ksize=[1, F, F, 1], strides=[1, 2, 2, 1], padding='VALID')

            # Second branch, 5x5 conv then global avg pool
            inception2a = self.convolution('Transition2', X, 5, K, S, padding, phase_train, BN, relu)
            inception2 = tf.nn.avg_pool2d(input=inception2a, ksize=[1, F, F, 1], strides=[1, 2, 2, 1], padding='VALID')

            # Third branch, 3x3 conv then global avg pool
            inception3a = self.convolution('Transition3', X, 3, K, S, padding, phase_train, BN, relu)
            inception3 = tf.nn.avg_pool2d(input=inception3a, ksize=[1, F, F, 1], strides=[1, 2, 2, 1], padding='VALID')
    ...
    def incepted_downsample(self, X, S=2, padding='SAME'):
        ...
        # 1st branch, AVG pool
        avg = tf.nn.avg_pool2d(input=X, ksize=[1, 2, 2, 1], strides=[1, S, S, 1], padding=padding)

        # 2nd branch, max pool
        maxi = tf.nn.max_pool2d(input=X, ksize=[1, 2, 2, 1], strides=[1, S, S, 1], padding=padding)
    ...
    def global_avg_pool(self, X, S=1, padding='VALID'):
        ...
        return tf.nn.avg_pool2d(input=X, ksize=[1, Fx, Fy, 1], strides=[1, S, S, 1], padding=padding)
    ...
    def segmentation_SCE_loss(self, logits, labelz, class_factor=1.2):
        ...
        # Output the summary
        if self.summary: tf.compat.v1.summary.scalar('loss', loss)
    ...
    def MSE_loss(self, logits, labels, mask_factor=0.0, mask_avg=0.0, mask_norm=0.0, debug=False):
        ...
        if mask_factor:
            MSE_loss = tf.reduce_mean(input_tensor=tf.multiply(tf.square(labels - logits), mask))
        else:
            MSE_loss = tf.reduce_mean(input_tensor=tf.square(labels - logits))

        # Output the summary of the MSE and MAE
        if self.summary:
            tf.compat.v1.summary.scalar('Square Error', MSE_loss)
            tf.compat.v1.summary.scalar('Absolute Error', tf.reduce_mean(input_tensor=tf.abs(labels - logits)))

        # Add these losses to the collection
        tf.compat.v1.add_to_collection('losses', MSE_loss)
    ...
    def SCE_loss(self, logits, labels, num_classes):
        ...
        # Output the summary of the MSE and MAE
        if self.summary: tf.compat.v1.summary.scalar('Cross Entropy', loss)
    ...
    def cost_sensitive_loss(self, logits, labels, loss_factor, num_classes):
        ...
        # Calculate  loss
        loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(tf.squeeze(labels)), logits=logits)
        ...
        # Output the summary of the MSE and MAE
        tf.compat.v1.summary.scalar('Cross Entropy', loss)
    ...
    def calc_L2_Loss(self, L2_gamma):
        ...
        weights = tf.compat.v1.get_collection('weights')

        # Sum the losses
        L2_loss = tf.multiply(tf.add_n([tf.nn.l2_loss(v) for v in weights]), L2_gamma)

        # Add it to the collection
        tf.compat.v1.add_to_collection('losses', L2_loss)

        # Activation summary
        if self.summary: tf.compat.v1.summary.scalar('L2_Loss', L2_loss)
    ...
    def pixel_wise_softmax(self, output_map):
        ...
        sum_exp = tf.reduce_sum(input_tensor=exponential_map, axis=3, keepdims=True)
        tensor_sum_exp = tf.tile(sum_exp, tf.stack([1, 1, 1, tf.shape(input=output_map)[3]]))

        # return the result of the softmax
        return tf.compat.v1.div(exponential_map, tensor_sum_exp)
    ...
    def DICE_loss(self, logitz, labelz, num_classes=2, network_dims=64):
        ...
        # Summary images
        if self.summary:
            tf.compat.v1.summary.image('Labels', tf.reshape(tf.cast(labels[2], tf.float32), shape=[1, network_dims, network_dims, 1]), 2)
            tf.compat.v1.summary.image('Logits', tf.reshape(logitz[2, :, :, 1], shape=[1, network_dims, network_dims, 1]), 2)
        ...
        intersection = 2 * tf.reduce_sum(input_tensor=logits * labels)

        # find the union
        union = eps + tf.reduce_sum(input_tensor=logits) + tf.reduce_sum(input_tensor=labels)

        # Calculate the loss
        dice = intersection / union

        # Output the training DICE score
        tf.compat.v1.summary.scalar('DICE_Score', dice)

        # 1-DICE since we want better scores to have lower loss
        loss = 1 - dice

        # Output the Loss
        tf.compat.v1.summary.scalar('Loss_Raw', loss)
    ...
    def batch_normalization(self, conv, phase_train, scope):
        ...
        return tf.compat.v1.layers.batch_normalization(conv, training=phase_train, momentum=0.98)
    ...
    def _activation_summary(self, x):
        ...
        tf.compat.v1.summary.histogram(x.op.name + '/activations', x)

        # " but with a scalar of the fraction of 0's
        tf.compat.v1.summary.scalar(x.op.name + '/sparsity', tf.nn.zero_fraction(x))
...
class SODLoss(object):
    ...
    def generalized_dice_loss(self, prediction, ground_truth, weight_map=None, type_weight='Square'):
        ...
        one_hot = self.labels_to_one_hot(ground_truth, tf.shape(input=prediction)[-1])
    ...
    def dice_plus_xent_loss(self, prediction, ground_truth, weight_map=None):
        ...
        if weight_map is not None:
            ...
            dice_numerator = 2.0 * tf.sparse.reduce_sum(one_hot * softmax_of_logits, axis=[0])
            dice_denominator = \
